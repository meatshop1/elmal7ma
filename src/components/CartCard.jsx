...
const CartCard = ({ product }) => {
  ...
  return (
    ...
    >
      ...
      >
        <p className={`text-white text-[0.9rem] pt-1 md:text-2xl leading-none mb-1 ${lng === "en" ? "" : "font-kufam"}`}>
          {lng === "en" ? product.product.name : product.product.name_ar}
        </p>
        <p className={`text-gray-500 text-xs md:text-sm ${lng === "en" ? "" : "font-kufam"}`}>
          {lng === "en" ? product.product.category_title : product.product.category_title_ar}
        </p>


        <Notes quantity={product.quantity} initialValue={product.notes} animal={product.animal} item_id={product.id}/>
